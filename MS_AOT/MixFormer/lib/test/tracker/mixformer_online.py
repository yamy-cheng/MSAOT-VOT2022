...
class MixFormerOnline(BaseTracker):
    ...
    def update_bbox(self, mixformer_box, aot_box):
        '''make new bbox that is mean of the two input box'''
        new_box = []
        for i in range(len(aot_box)):
            new_box.append((mixformer_box[i] + aot_box[i]) / 2)

        self.state = new_box

    def track_without_update(self, image, info: dict = None):
        H, W, _ = image.shape
        self.frame_id += 1
        '''
        self.state is previous pred_bbox when call this function 
        '''
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image, self.state, self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h)
        search = self.preprocessor.process(x_patch_arr)
        ...
        '''
        self.state in return is current_frame pred_bbox as pred_score
        '''
        if self.save_all_boxes:
            '''save all predictions'''
            all_boxes = self.map_box_back_batch(pred_boxes * self.params.search_size / resize_factor, resize_factor)
    ...
    def update_online_template(self, state, pred_score, image):
        '''
        the state is the pred_bbox of current_frame 
        this function should be called after get the pred_bbox of current_frame
        '''
    ...
    def update_template(self, state, image, conf, valid):
        self.state = state
        
        if conf > 0.7 and valid:
