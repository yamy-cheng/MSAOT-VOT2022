...
class Tracker:
    ...
    def run_seq(self, gt_path, optional_box=None, debug=None, visdom_info=None, save_results=False):
        ...
        seq_path = '/home/yoxu/vot_2022/sequences/agility/color'
        seq_list = os.listdir(seq_path)
        seq_list = seq_list[1:]
        seq_list = sorted(seq_list)
        for seq_name in seq_list:
            frame = cv.imread(os.path.join(seq_path, seq_name))
            ...
            cv.rectangle(frame_disp, (state[0], state[1]), (state[2] + state[0]*2, state[3] + state[1]*2),
                         (0, 255, 0), 5)

            from PIL import Image
            temp = Image.fromarray(frame_disp)
            temp.save("/home/yoxu/vot_2022/MixFormer/current_img.jpg")
